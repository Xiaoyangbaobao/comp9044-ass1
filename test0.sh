...
cd "$ref_dir" || exit 1
2041 mygive-add lab03_scraping_courses scraping_courses.sh scraping_courses.tests scraping_courses.marking > "$expected_stdout" 2> "$expected_stderr"
ref_exit_code=$?

cd "$test_dir" || exit 1
mygive-add lab03_scraping_courses scraping_courses.sh scraping_courses.tests scraping_courses.marking > "$actual_stdout" 2> "$actual_stderr"
exit_code=$?
