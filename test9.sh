...
#test mygive-submit and check line problem
...
cd "$ref_dir" || exit 1
2041 mygive-add  ass_test stderr.test > "$expected_stdout" 2> "$expected_stderr"
ref_exit_code=$?

cd "$test_dir" || exit 1
mygive-add  ass_test stderr.test > "$actual_stdout" 2> "$actual_stderr"
exit_code=$?

# make first submission
cd "$ref_dir" || exit 1
2041 mygive-submit ass_test z5123456 stderr_no_newline.sh > "$expected_stdout" 2> "$expected_stderr"
ref_exit_code=$?

cd "$test_dir" || exit 1
./mygive-submit ass_test z5123456 stderr_no_newline.sh > "$actual_stdout" 2> "$actual_stderr"
exit_code=$?

cd "$ref_dir" || exit 1
2041 mygive-mark ass_test > "$expected_stdout" 2> "$expected_stderr"
ref_exit_code=$?

cd "$test_dir" || exit 1
./mygive-mark ass_test > "$actual_stdout" 2> "$actual_stderr"
exit_code=$?
